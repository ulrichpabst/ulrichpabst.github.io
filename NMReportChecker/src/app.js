...
function simulateSpectrum(text, ppmMin, ppmMax, points, gammaBase) {
  ...
  const solvDef = residualSolventDef(parsed.solvent);
  if (solvDef) {
    const tot = Math.max(1e-9, hsum(entries));
    const nHsolv = Math.max(0.3, Math.min(1.2, 0.06 * tot));
    entries.push({
      center: solvDef.ppm,
      halfw: 0,
      range: null,
...
const IMP = {
  "CDCl3": [
    { name: "CHCl3",             ppm: 7.26, mult: "s" },
    { name: "H2O",               ppm: 1.56, mult: "s" },
    { name: "Acetone",           ppm: 2.17, mult: "s" },
    { name: "Acetic acid",       ppm: 2.10, mult: "s" },
    { name: "Ethanol CH3",       ppm: 1.25, mult: "t" },
    { name: "Ethanol CH2",       ppm: 3.72, mult: "q" },
    { name: "Ethyl acetate CH3", ppm: 1.26, mult: "t" },
    { name: "Ethyl acetate CH2", ppm: 4.12, mult: "q" },
    { name: "Diethyl ether CH3", ppm: 1.21, mult: "t" },
    { name: "Diethyl ether CH2", ppm: 3.48, mult: "q" },
    { name: "DMF CH",            ppm: 8.02, mult: "s" },
    { name: "DMF CH3",           ppm: 2.88, mult: "s" },
    { name: "DMF CH3",           ppm: 2.96, mult: "s" },
    { name: "Acetonitrile",      ppm: 2.10, mult: "s" },
    { name: "Toluene CH3",       ppm: 2.36, mult: "s" },
...
function residualSolventDef(solvent) {
  if (solvent === "CDCl3") return { ppm: 7.26, multSeq: ['s'], multRaw: 's' };
  if (solvent === "DMSO-d6") return { ppm: 2.50, multSeq: ['s'], multRaw: 's' };
  if (solvent === "C6D6") return { ppm: 7.16, multSeq: ['s'], multRaw: 's' };
  if (solvent === "D2O") return { ppm: 4.79, multSeq: ['s'], multRaw: 's' };
...
function likelyIdentity(ppm, mult, solvent) {
  ...
  for (const it of list) {
    ...
    if (matchMult && Math.abs(ppm - it.ppm) <= tol) {
      return { label: " " + it.name, isImp: true };
    }
  }
...
function buildTable(entries, solvent) {
  ...
  entries.forEach((e, idx) => {
    ...
    let identHTML = "—";
    let isImp = false;
    const isSolv = !!e.isSolvent;

    if (isSolv) {
      identHTML = `<span class="badge solv">SOLV</span> ${solvent || '—'}`;
    } else {
      const id = likelyIdentity(e.center, multForMatch, solvent);
      isImp = id.isImp;
      identHTML = isImp ? `<span class="badge imp">IMP</span> ${id.label}` : id.label;
    }

    if (isImp) { impIdx.push(idx); impCount++; }
    if (isSolv) { solvIdx.push(idx); }

    const tr = document.createElement('tr');
    if (isImp) tr.classList.add('impurity');
    if (isSolv) tr.classList.add('solvent');
  ...
  return { impCount, impIdx, solvIdx };
}
...
if (inAxisBand || e.shiftKey || e.button === 1 || e.button === 2) {
  dragType = 'pan';
} else if (cross) {
  dragType = 'measure';
  selBand = [pixToPpm(x), pixToPpm(x)];
  if (deltaEl){ deltaEl.classList.add('floating'); deltaEl.classList.remove('hidden'); }
} else {
  dragType = 'zoom';
}
drawOverlay();
...
overlay.addEventListener('mousemove', e => {
  ...
  if (drag && dragType === 'measure') {
  drag.x1 = hover.x;
  const ppmA = pixToPpm(drag.x0);
  const ppmB = pixToPpm(drag.x1);
  const loSel = Math.min(ppmA, ppmB);
  const hiSel = Math.max(ppmA, ppmB);
  selBand = [loSel, hiSel];
  const MHz = (lastSim && isFinite(lastSim.MHz)) ? lastSim.MHz : 400;
  const dppm = Math.abs(hiSel - loSel);
  const dhz  = dppm * MHz;
  if (deltaEl) {
    const rect = overlay.getBoundingClientRect();
    deltaEl.textContent = `Δ ${dppm.toFixed(4)} ppm | ${dhz.toFixed(1)} Hz`;
    deltaEl.style.left = `${(hover.x / dpr) + rect.left - rect.left + 12}px`;
    deltaEl.style.top  = `${(hover.y / dpr) + rect.top  - rect.top  + 12}px`;
    deltaEl.style.right = '';
  }
}
  else {
...
if (dragType === 'zoom') {
  ...
  if (xB - xA > 10 && drag.y0 >= yMin && drag.y0 <= yMax) {
    ...
    if (xrange[0] > xrange[1]) xrange = [hiSel, loSel]; else xrange = [loSel, hiSel];
    draw();
  }
} else if (dragType === 'measure') {
  selBand = null;
...
window.addEventListener('keydown', e => {
  ...
  if (e.key === 'c' || e.key === 'C') {
    cross = !cross;
    if (!cross && deltaEl){ deltaEl.classList.add('hidden'); }
    drawOverlay();
  }
});
...
function pixToPpm(px){
  const w = canvas.width - pxL() - pxR();
  const t = (px - pxL()) / Math.max(1, w);
  return lerp(xrange[0], xrange[1], t);
}
...
function drawBands() {
  ...
  if (solvBands.length) {
  for (const b of solvBands) {
    const x1 = xToPix(b[0]);
    const x2 = xToPix(b[1]);
    ov.fillStyle = cssVar('--solvOverlay', cssVar('--solvTable', '#dbeafe'));
...
function drawOverlay() {
  ...
  if (drag && (dragType === 'zoom' || dragType === 'measure')) {
...
function setData(d) {
  ...
  return { setData, draw, setHighlight, clearHighlight, setImpBands, setSolvBands, xToPix, yToPix, LINE_W, getXrange: () => xrange, getDefaultX: () => defaultX };
}
...
const solvBands = res.solvIdx.map(i => signalBand(lastEntries[i], sim.MHz, lastGamma));
renderer.setImpBands(impBands);
renderer.setSolvBands(solvBands);
  } catch (e) {
...
if (inset) {
  inset.addEventListener('mousemove', e => {
...
const t = document.getElementById('themeToggle') || document.getElementById('theme-toggle');
if (t) {
